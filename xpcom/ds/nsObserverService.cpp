...
#define NS_ENSURE_VALIDCALL \
  ...
  NS_IMETHODIMP
nsObserverService::AddObserverOnce(nsIObserver* aObserver, const char* aTopic,
                               bool aOwnsWeak)
{
  LOG(("nsObserverService::AddObserverOnce(%p: %s)",
  ...
  return observerList->AddObserverOnce(aObserver, aOwnsWeak);
}
