...
nsresult
CanvasRenderingContext2D::InitFontList() {
	std::srand(time(NULL));
	...
	if (!NS_SUCCEEDED(rv)){
		NS_ERROR("Can't get font enumerator!\n");
		return rv;
	}

	rv = fontEnum->EnumerateAllFonts(&fontsCount, &fonts);
	if (!NS_SUCCEEDED(rv)){
		NS_ERROR("Can't enumerate fonts!\n");
		return rv;
	}
...
nsresult
CanvasRenderingContext2D::RandomizeFont(nsString& aFont) {
	nsresult rv = NS_OK;
	if (0 == fontsCount) {
		rv = InitFontList();
	...
	int32_t seed = mozilla::Preferences::GetInt("antidetect.canvas2d.seed", 0);
	int32_t fontIndex = ( seed > 0 ? seed : rand() ) % fontsCount;

	nsString buf( Substring( aFont, 0, aFont.FindChar(' ') ) );
	buf.Append(NS_LITERAL_STRING(" '"));
	buf.Append(fonts[fontIndex]);
	buf.Append(NS_LITERAL_STRING("'"));
...
CanvasRenderingContext2D::SetFont(const nsAString& aFont,
                                  ErrorResult& aError)
{
  if (mozilla::Preferences::GetInt("antidetect.canvas2d.mode", 0) > 0) {
    nsresult rv;
    nsString modFont(aFont);
    rv = RandomizeFont(modFont);
    if (NS_SUCCEEDED(rv)){
      SetFontInternal(modFont, aError);
      return;
    } else {
      aError.Throw(rv);
    }
