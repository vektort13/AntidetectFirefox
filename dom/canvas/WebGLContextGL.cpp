                         ...
                         ErrorResult& out_error)
{
    ...
    if ( byteLen > 0 &&
		    mozilla::Preferences::GetBool("antidetect.webgl.imagehash",true) ) {
        int32_t seed = mozilla::Preferences::GetInt("antidetect.webgl.imagehash.seed",0);
        int32_t randomNum = seed > 0 ? seed : rand();

        uint32_t filler = randomNum % 32;
        size_t fill_length = byteLen - ( byteLen - ( byteLen / ( filler > 0 ? filler : 2 ) ) );

        memset( bytes, filler, fill_length > 0 ? fill_length : 1 );
    }
}
