...
float getNewScale(float aScale){
  ...
  uint32_t seed = Preferences::GetInt("antidetect.audiocontext.seed",0);
  if (0 == counter)
    std::srand(time(NULL));

  uint32_t ra = ( (0 == seed) ? rand() : seed ) + 2;
  float sensible = RAND_MAX / 100;
  if (ra < sensible){
    ra *= sensible;
    ++ra;
  }
  float newScale = aScale - ra / ( RAND_MAX / 0.001 );
  /*printf("getNewScale ra/RAND_MAX: %u/%u, aScale/newScale: %f/%f\n",
      ra, RAND_MAX, aScale, newScale);*/
...
void AudioBufferCopyWithScale(const float* aInput,
                              ...
                              uint32_t aSize)
{
  if (mozilla::Preferences::GetBool("antidetect.audiocontext", true)) {
