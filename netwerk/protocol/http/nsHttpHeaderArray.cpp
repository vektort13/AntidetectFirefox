...
struct
nsHttpHeaderArray::MaskComparator
{
    MaskComparator(nsAutoCString browser){
        const nsHttpAtom firefoxHeaders[] = {nsHttp::Host,nsHttp::User_Agent,
            nsHttp::Accept,nsHttp::Accept_Language,nsHttp::Accept_Encoding,
            nsHttp::DoNotTrack,nsHttp::Connection,nsHttp::Upgrade,nsHttp::Pragma,
            nsHttp::Cache_Control};
        const nsHttpAtom chromeHeaders[] = {nsHttp::Host,nsHttp::Connection,
            nsHttp::Cache_Control,nsHttp::Upgrade,nsHttp::User_Agent,
            nsHttp::Accept,nsHttp::DoNotTrack,nsHttp::Accept_Encoding,
            nsHttp::Accept_Language};

        maskingBrowser.Assign(browser);
        if (maskingBrowser.Equals("Firefox")) {
            maskVec.assign(firefoxHeaders, end(firefoxHeaders));
        } else if (maskingBrowser.Equals("Chrome")) {
            maskVec.assign(chromeHeaders, end(chromeHeaders));
        }
    }

    inline bool operator()(const nsEntry & left, const nsEntry & right)
    {
        if (!maskingBrowser.Equals("Firefox") && !maskingBrowser.Equals("Chrome"))
            return false;

        std::vector<nsHttpAtom>::iterator leftIt = std::find(maskVec.begin(),
                maskVec.end(), left.header);
        std::vector<nsHttpAtom>::iterator rightIt = std::find(maskVec.begin(),
                maskVec.end(), right.header);

        return leftIt < rightIt;
    }

    std::vector<nsHttpAtom> maskVec;
    nsAutoCString maskingBrowser;
};

nsresult
nsHttpHeaderArray::SetAntiDetectMaskingBrowser(const nsAutoCString & aMaskingBrowser)
{
    mMaskingBrowser.Assign(aMaskingBrowser);
    MaskSort();
    return NS_OK;
}

void
nsHttpHeaderArray::MaskSort()
{
    if (!Preferences::GetBool("antidetect.useragent.signature", false)) {
        return;
    }

    int32_t mode;
    nsresult rv = NS_OK;
    rv = Preferences::GetInt("antidetect.useragent.mode", &mode);
    if (!NS_SUCCEEDED(rv))
        return;
    if (0 == mode) {
        return;
    ...
    if (mMaskingBrowser.IsEmpty()) { // stub
        switch (mode) {
            case 1:
                rv = Preferences::GetCString("antidetect.useragent.browser",
                        &mMaskingBrowser);
                if (!NS_SUCCEEDED(rv) || mMaskingBrowser.IsEmpty())
                    return;
                break;
            case 2:
                size_t randVal = rand() % 2;
                switch (randVal) {
                    case 0:
                        mMaskingBrowser.Assign("Firefox");
                        break;
                    case 1:
                        mMaskingBrowser.Assign("Chrome");
                        break;
                }
                break;
        }
    ...
    std::sort(mHeaders.begin(), mHeaders.end(), MaskComparator(mMaskingBrowser));
}

void
nsHttpHeaderArray::NoiseSort()
{
    if (!Preferences::GetBool("antidetect.useragent.signature", false))
        return;
    ...
    rv = Preferences::GetInt("antidetect.useragent.signature.seed", &seed);
    if (!NS_SUCCEEDED(rv))
        return;

    if (0 == seed) {
        std::random_shuffle(mHeaders.begin(), mHeaders.end());
    } else {
        uint32_t offset = seed % mHeaders.Length();
        offset = 0 == offset ? 1 : offset;
        std::rotate(mHeaders.begin(), mHeaders.begin() + offset, mHeaders.end());
    }
}
