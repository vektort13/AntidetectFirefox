...
{
  LOG(("HttpBaseChannel::SetDocshellUserAgentOverride called..."));
  ...
  if (!loadContext) {
    LOG(("HttpBaseChannel::SetDocshellUserAgentOverride can't get context..."));
    return NS_OK;
  }
  ...
  if (!domWindow) {
    LOG(("HttpBaseChannel::SetDocshellUserAgentOverride can't get dom window..."));
    return NS_OK;
  }
  ...
  if (!docshell) {
    LOG(("HttpBaseChannel::SetDocshellUserAgentOverride can't get docShell..."));
    return NS_OK;
  }

  nsAutoCString maskingBrowser;
  rv = docshell->GetAntiDetectMaskingBrowser(maskingBrowser);
  if (NS_SUCCEEDED(rv)) {
    mRequestHead.SetAntiDetectMaskingBrowser(maskingBrowser);
  }
